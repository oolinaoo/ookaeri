...
$(function () {
	$.ajax({
	  url: "/okaeri/forumMessage/msgJoinArtJoinType",
	  type: "GET",
	  data: "",
	  dataType: "json",
	  success: function (data) {
		...
		$.each(data, function (i, item) {
		  list_html += `
						  <tr>
						  <td class='id'>${item.forMsgNo}</td>
						  <td class='content'>${item.forMsgContent}</td>
						  <td class='artId'>${item.forArtNo}</td>
		...
		$.each(data, function (i, item) {
		  	$.each(data, function (j, obj) {
				if (data[i].forArtNo == data[j].forArtNo) {
					$(`.dataArray${i} h3`).after(
						`
						<div class='article_message'>
							<div>${obj.memAcctMsg}
							${obj.forMsgContent}
							<span style='float: right; font-size: 0.8em'>${obj.forMsgPosttime}</span>
							</div>
						</div>
						`
					);
