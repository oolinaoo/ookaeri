...
// Ajax forArtNo
function init() {
	let url = "/okaeri/forumArticle/artNoJoinMsg";
	let art_data = {
		"forArtNo" : artNo
	}
	...
	xhr.onload = function() {
		...
		$(".forum-row-box")
				.append(
						"<div class='forum-title'>"
								+ "<span style='float:right;'><a href='#' id='forum-heart' onclick='collection(this)'><i class='fa fa-heart dataHeart" + artNo + "'></i></a>"
								+ "</span>" + "<h3>"
								+ data[0].forArtTitle
								+ "</h3>"
								+ "<p>"
		...
		for (let i = 0; i < data[0].message.length; i++) {
			if (data[0].message[i].forMsgContent != null
					&& data[0].message[i].forMsgState != 1) {
				$(".refresh-comment")
						.prepend(
...
$.ajax({
    ...
    success: function (hdata) {
      $.each(hdata, function (k, item) {
			if(`${item.forArtNo}` == artNo) {
				$(`.dataHeart${artNo}`).css("color", "rgb(123, 20, 20)")
			}
		});
    },
    error: function (xhr) {
...
function report(obj) {
	if (prompt("確定要檢舉嗎？確定請輸入yes") == "yes") {
		...
		let reason = $(obj).parent().siblings(".inputStyle").find("input")
				.val();
		let mem = "gina3";
		console.log(art);
		console.log(msg);
